...
use App\Http\Controllers\NfeInsurerController;
...
Route::get('/nfe/seguradoras', [NfeInsurerController::class, 'index'])->name('nfe.insurers.index');
Route::get('/nfe/seguradoras/nova', [NfeInsurerController::class, 'create'])->name('nfe.insurers.create');
Route::post('/nfe/seguradoras', [NfeInsurerController::class, 'store'])->name('nfe.insurers.store');
Route::get('/nfe/seguradoras/{insurer}/editar', [NfeInsurerController::class, 'edit'])->name('nfe.insurers.edit');
Route::put('/nfe/seguradoras/{insurer}', [NfeInsurerController::class, 'update'])->name('nfe.insurers.update');
...
Route::post('/nfe/produtos-seguro', [NfeInsuranceProductController::class, 'store'])->name('nfe.insurance-products.store');
    ...
    Route::get('/settings', function () {
        $user = auth()->user();
        if (! $user || ! in_array((int) $user->funcao, [7, 0, 1], true)) {
            abort(403);
        }

        return Inertia::render('Settings/Config');
    })->name('settings.config');
    Route::put('/settings/billing-plans', [BillingPlanController::class, 'update'])
