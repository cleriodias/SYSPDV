...
class ProductDiscardController extends Controller
{
    public function index(Request $request): Response
    {
        ...
        $recent = ProductDiscard::query()
            ->with('product:tb1_id,produto_id,tb1_nome,tb1_codbar,tb1_vlr_venda')
            ...
            ->map(function (ProductDiscard $discard) {
                return [
                    ...
                    'product' => $discard->product
                        ? [
                            'id' => $discard->product->produto_id ?? $discard->product->tb1_id,
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        if (! $product && ctype_digit((string) $data['product_id'])) {
            $product = Produto::query()
                ->forMatrix($this->resolveMatrixId($request))
        ...
        if (! $product) {
            return redirect()
                ->back()
                ->withErrors([
                    'product_id' => 'O produto selecionado nao foi encontrado nesta matriz.',
                ]);
        }
