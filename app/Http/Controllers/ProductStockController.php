...
class ProductStockController extends Controller
{
    ...
    public function index(Request $request): Response
    {
        ...
        $selectedProductId = $productionProducts->contains('produto_id', $selectedProductId)
            ? $selectedProductId
            : null;

        $recentMovements = ProductStockMovement::query()
            ->with([
                'product:tb1_id,produto_id,tb1_nome,tb1_codbar',
                'user:id,name',
            ])
            ...
            ->map(fn (ProductStockMovement $movement) => [
                ...
                'product' => $movement->product
                    ? [
                        'id' => $movement->product->produto_id ?? $movement->product->tb1_id,
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        DB::transaction(function () use ($data, $request) {
            ...
            $product = Produto::query()
                ...
                ->where('produto_id', (int) $data['product_id'])
                ->lockForUpdate()
                ->first();
            ...
            if (! $product) {
                $product = Produto::query()
                    ->forMatrix($this->resolveMatrixId($request))
                    ->where('tb1_id', (int) $data['product_id'])
                    ->lockForUpdate()
                    ->firstOrFail();
            }
        ...
        return redirect()
            ->route('products.production-stock', ['product_id' => $product->produto_id ?? $data['product_id']])
