...
class SaleController extends Controller
{
    ...
    public function store(Request $request, FiscalInvoicePreparationService $fiscalInvoicePreparationService): JsonResponse
    {
        ...
        foreach (($validated['items'] ?? []) as $item) {
            $requestedProductId = (int) $item['product_id'];
            ...
            if ($weightedBarcode !== null && $weightedBarcode['product_id'] !== $requestedProductId) {
                throw ValidationException::withMessages([
                    'items' => 'O codigo de barras da balanca nao corresponde ao produto informado.',
                ]);
            }

            $product = $this->resolveProductByRequestedCode($request, $requestedProductId, ['tb1_id', 'produto_id']);

            if (! $product) {
                throw ValidationException::withMessages([
                    'items' => 'Um dos produtos informados nao foi encontrado na matriz ativa.',
                ]);
            }

            $unitPrice = $weightedBarcode['unit_price'] ?? $explicitUnitPrice;
            $itemKey = $this->buildSaleItemKey((int) ($product->produto_id ?? $requestedProductId), $unitPrice);

            if (! isset($groupedItems[$itemKey])) {
                $groupedItems[$itemKey] = [
                    'product_id' => (int) ($product->produto_id ?? $requestedProductId),
                    'internal_product_id' => (int) $product->tb1_id,
                    'quantity' => 0,
        ...
        if ($comandaCodigo) {
            ...
            $comandaPayload = $comandaItems
                ->groupBy(fn ($item) => $this->buildSaleItemKey(
                    (int) ($item->produto?->produto_id ?? $item->tb1_id),
                ...
                ->map(function ($group) {
                    ...
                    return [
                        'product_id' => (int) ($first->produto?->produto_id ?? $first->tb1_id),
                        'internal_product_id' => (int) $first->tb1_id,
            ...
            if (! empty($groupedItems)) {
                $products = Produto::query()
                    ...
                    ->whereIn('produto_id', array_values(array_unique(array_map(
                        fn (array $item) => $item['product_id'],
                        $groupedItems
                    ))))
                    ->get(['tb1_id', 'produto_id', 'tb1_nome', 'tb1_vlr_venda', 'tb1_vr_credit'])
                    ->keyBy('produto_id');
                ...
                foreach ($groupedItems as $itemKey => $requestedItem) {
                    ...
                    if ($existingItem) {
                        if ($requestedQuantity > $existingQuantity) {
                            ...
                            'internal_product_id' => $existingItem['internal_product_id'] ?? null,
                        ...
                        $finalItemsMap[$itemKey] = [
                            'product_id' => (int) ($product->produto_id ?? $productId),
                            'internal_product_id' => (int) $product->tb1_id,
            ...
            if ($requiresVrEligible) {
                $eligibility = Produto::query()
                    ->forMatrix($this->resolveMatrixId($request))
                    ->whereIn('produto_id', array_column($itemsPayload, 'product_id'))
                    ->pluck('tb1_vr_credit', 'produto_id');
            ...
            foreach ($groupedItems as $groupedItem) {
                ...
                $product = $this->resolveProductByRequestedCode(
                    $request,
                    $productId,
                    ['tb1_id', 'produto_id', 'tb1_nome', 'tb1_vlr_venda', 'tb1_vr_credit']
                );
        ...
        if ($comandaCodigo) {
            ...
            foreach ($extraItems as $item) {
                Venda::create([
                    'tb4_id' => $payment->tb4_id,
                    'tb1_id' => $item['internal_product_id'] ?? $item['product_id'],
    ...
    public function addComandaItem(Request $request, int $codigo): JsonResponse
    {
        ...
        $product = $this->resolveProductByRequestedCode(
            ...
            ['tb1_id', 'produto_id', 'tb1_nome', 'tb1_vlr_venda']
        );

        if (! $product) {
            return response()->json(['message' => 'Produto nao encontrado na matriz ativa.'], 422);
        }
        ...
        if ($weightedBarcode !== null && $weightedBarcode['product_id'] !== (int) ($product->produto_id ?? $product->tb1_id)) {
            return response()->json([
    ...
    public function updateComandaItem(Request $request, int $codigo, string $lineKey): JsonResponse
    {
        ...
        $resolvedProduct = $this->resolveProductByRequestedCode(
            $request,
        ...
        if (! $resolvedProduct) {
            return response()->json(['message' => 'Produto nao encontrado na matriz ativa.'], 404);
        }

        $lineData['internal_product_id'] = (int) $resolvedProduct->tb1_id;

        $record = Venda::query()
            ->where('id_comanda', $codigo)
            ->where('id_unidade', $unitId)
            ->where('tb1_id', $lineData['internal_product_id'])
    ...
    private function getComandaItems(int $codigo, ?int $unitId = null): array
    {
        $items = Venda::query()
            ...
            ->with('produto:tb1_id,produto_id')
        ...
        return $items
            ...
            ->map(function ($group) use ($lancUsers) {
                ...
                $logicalProductId = (int) ($first->produto?->produto_id ?? $first->tb1_id);
                $lineId = $this->buildSaleItemKey($logicalProductId, $price);

                return [
                    'line_id' => $lineId,
                    'id' => $logicalProductId,
                    'product_id' => $logicalProductId,
    ...
    private function resolveMatrixId(Request $request): ?int
    {
        $unitId = $this->resolveActiveUnitId($request);

        if ($unitId) {
            $matrixId = Unidade::query()->where('tb2_id', $unitId)->value('matriz_id');
        ...
        $userMatrixId = (int) ($request->user()?->matriz_id ?? 0);

        return $userMatrixId > 0 ? $userMatrixId : null;
    }

    private function resolveProductByRequestedCode(Request $request, int $requestedProductId, array $columns = ['*']): ?Produto
    {
        ...
        $product = (clone $query)
            ->where('produto_id', $requestedProductId)
            ->first();
        ...
        return $query
            ->where('tb1_id', $requestedProductId)
            ->first();
    }
    ...
    {
        $resolvedUnitId = (int) ($unitId ?? $record->id_unidade ?? 0);
        $systemUser = $this->ensureSystemUser($resolvedUnitId > 0 ? $resolvedUnitId : null);
        $recipientIds = User::query()
            ->where(function ($query) use ($resolvedUnitId) {
                $query->where('funcao_original', 0)
                    ->orWhere(function ($managerQuery) use ($resolvedUnitId) {
                        $managerQuery->where('funcao_original', 1)
                            ->where(function ($unitQuery) use ($resolvedUnitId) {
                                $unitQuery->where('tb2_id', $resolvedUnitId)
                                    ->orWhereHas('units', function ($relationQuery) use ($resolvedUnitId) {
                                        $relationQuery->where('tb2_unidades.tb2_id', $resolvedUnitId);
                                    });
                            });
                    });
            })
        ...
        foreach ($recipientIds as $recipientId) {
            ChatMessage::create([
                ...
                'sender_unit_id' => $resolvedUnitId > 0 ? $resolvedUnitId : null,
                'message' => $message,
            ]);
        }
    }
    ...
    private function buildComandaItemRemovalMessage(User $executor, Venda $record, string $removalReason, Carbon $removedAt): string
    {
        ...
        $lines = [
            '[b]Aviso automatico do sistema - exclusao de item da comanda[/b]',
            'Registro gerado automaticamente pelo terminal da lanchonete.',
            sprintf('Responsavel pela exclusao: %s', $executor->name),
            ...
            sprintf('Motivo informado: %s', $removalReason),
        ];
