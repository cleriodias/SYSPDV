...
class UnitController extends Controller
{
    private function ensureAuthorized(): void
    {
        ...
        if (!$user || !in_array((int) $user->funcao, [7, 0, 1])) {
    ...
    public function index(): Response
    {
        ...
        if (ManagementScope::isBoss($user)) {
            // Boss visualiza todas as matrizes.
        } elseif (ManagementScope::isMaster($user)) {
            $unitsQuery->where('matriz_id', ManagementScope::scopedMatrixId($user));
        } elseif (ManagementScope::isManager($user)) {
