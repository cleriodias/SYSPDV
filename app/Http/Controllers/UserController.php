...
class UserController extends Controller
{
    ...
    public function index(Request $request): Response
    {
        ...
        return Inertia::render('Users/UserIndex', [
            ...
            'filters' => $filters,
            'permissions' => [
                'canCreate' => in_array((int) $authUser->funcao, [7, 0, 1], true),
                'canView' => in_array((int) $authUser->funcao, [7, 0, 1], true),
                'canEdit' => in_array((int) $authUser->funcao, [7, 0, 1], true),
                'canDelete' => in_array((int) $authUser->funcao, [7, 0, 1], true),
                'canManageSalaryAdvances' => in_array((int) $authUser->funcao, [7, 0, 1, 2], true),
            ],
        ]);
    }
    ...
    public function store(Request $request)
    {
        ...
        $request->validate(
            ...
            [
                ...
                'tb2_id.required' => 'Selecione uma unidade.',
        ...
        $matrixId = ManagementScope::scopedMatrixId($authUser);

        if (! ManagementScope::isBoss($authUser) && $matrixId <= 0) {
            abort(403);
        }

        $accessCode = $this->generateUniqueAccessCode($request->password);
        ...
        $user = User::create([
            ...
            'tb2_id' => $primaryUnit,
            'matriz_id' => ManagementScope::isBoss($authUser)
                ? Unidade::query()->where('tb2_id', $primaryUnit)->value('matriz_id')
                : $matrixId,
        ]);
    ...
    public function update(Request $request, User $user)
    {
        ...
        $primaryUnit = $unitIds[0] ?? 1;
        $matrixId = ManagementScope::isBoss($request->user())
            ? Unidade::query()->where('tb2_id', $primaryUnit)->value('matriz_id')
            : ManagementScope::scopedMatrixId($request->user());
