...
export default function UpdateProfileInformationForm({ className = '' }) {
    ...
    const units =
        (Array.isArray(user.units) && user.units.length > 0
            ...
            ? [user.tb2_id]
            : []) || [];
    const matrixName = user.matriz?.nome ?? '---';
    ...
    return (
        <section className={`space-y-4 ${className}`}>
            ...
            <form onSubmit={submit} className="space-y-4">
                ...
                <div className="rounded-xl border border-gray-200 bg-gray-50 p-4">
                    <p className="text-xs font-semibold uppercase text-gray-500">Matriz</p>
                    <p className="text-base font-semibold text-gray-800">{matrixName}</p>
                </div>
