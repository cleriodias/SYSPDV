...
export default function ProductionStock({
    ...
    const defaultProductId = selectedProductId ?? productionProducts[0]?.produto_id ?? productionProducts[0]?.tb1_id ?? "";
    ...
    const selectedProduct = productionProducts.find(
        (product) => Number(product.produto_id ?? product.tb1_id) === Number(data.product_id)
    );
                                ...
                                <div>
                                    ...
                                    >
                                        {productionProducts.map((product) => (
                                            <option key={product.tb1_id} value={product.produto_id ?? product.tb1_id}>
                                                {product.tb1_nome}
                                            </option>
                                        ))}
                                    </select>
                                            ...
                                            <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                                                {productionProducts.map((product) => (
                                                    <tr key={product.tb1_id}>
                                                        ...
                                                        <td className="px-3 py-2">{product.produto_id ?? product.tb1_id}</td>
                                                        <td className="px-3 py-2 text-right">{Number(product.tb1_qtd ?? 0)}</td>
                                                        <td className="px-3 py-2 text-center">
                                                            <PrimaryButton
                                                                type="button"
                                                                onClick={() => setData("product_id", product.produto_id ?? product.tb1_id)}
