...
export default function UserEdit({ auth, user, units = [] }) {
    ...
    const initialUnit = user.tb2_id
        ? String(user.tb2_id)
        : (user.units && user.units.length ? String(user.units[0].tb2_id) : '');
                                            ...
                                            return (
                                                <label key={unit.tb2_id} className="inline-flex items-center space-x-2 text-sm text-gray-700 dark:text-gray-200">
                                                    <input
                                                        ...
                                                        name="tb2_id"
                                                        value={unitId}
                                                        checked={data.tb2_id === unitId}
                                                        onChange={(e) => setData('tb2_id', e.target.value)}
